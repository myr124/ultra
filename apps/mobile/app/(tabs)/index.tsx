...
export default function TodayScreen() {
    ...
    const nextTask =
        recommendations.find((recommendation) => recommendation.startMinute >= currentMinute) ?? recommendations[0];
            ...
            <SurfaceCard accent={accentMap[nextTask.type]}>
                <CardHeader className="gap-3 px-5 py-5">
                    <View className="flex-row items-center justify-between gap-3">
                        ...
                        <Text className="text-sm text-muted-foreground">{nextTask.timeRange}</Text>
                    </View>
                    <CardTitle className="text-2xl leading-7">{nextTask.title}</CardTitle>
                </CardHeader>
                <CardContent className="px-5 pb-5">
                    <Text className="leading-5 text-muted-foreground">{nextTask.rationale[0]}</Text>
                </CardContent>
            </SurfaceCard>
        </ScrollView>
    );
